    ...
    for i in tqdm(range(nepochs)):
        ...
        for Xtrain in train_loader:    
            ...
            if args['probType'] == 'dc_wss':
                ...
                Yhat_train = solver_net.process_output_differentiable(Xtrain, Yhat_train) # Process output for differentiable loss            
                Yhat_train = solver_net.process_output_sorted(Yhat_train)
        ...
        ########################################################################
            ...
            if FIXED_Y_VALUE:
                ...
                Yhat_train = torch.tensor(
                    [
                    [11.0394, 11.3124, 11.3400, 11.4856, 13.5568,  3.9546,  2.2934,  4.6573, 2.9885,  2.6506],
                                                
                    [ 1.2621,  8.2478, 11.8904, 12.0048, 12.4628,  0.7087,  4.2336,  2.4109, 4.0444,  2.1954],

                     [ 8.2433,  9.3392, 14.2442, 15.7811, 17.8647,  1.2968,  0.5593,  1.5385, 2.5607,  1.2267],

                     [ 7.9705, 10.4392, 10.9540, 12.0336, 14.3340,  2.3835,  1.6596,  2.6133, 2.6821,  1.7713],

                    [ 2.8068, 10.2983, 16.2900, 19.1183, 21.8492,  1.9350,  4.7681,  3.9097,1.8972,  3.7358],
                    ...
                    [ 4.3271,  7.6628,  7.7788,  8.4799, 17.8769,  3.6407,  3.9312,  3.3874,
          1.0554,  3.8233]
            ...
            ynew_start_time = time.time()

            Ynew_train = grad_steps(data, Xtrain, Yhat_train, args, epoch=i) # 1. Forward pass                             
            # Generate GIF            


            print('Tempo Ynew ', time.strftime("%H:%M:%S", time.gmtime(time.time() - ynew_start_time)))
...
########################################################################            
            ...
            print('Tempo train loss ', time.strftime("%H:%M:%S", time.gmtime(time.time() - train_loss_start_time)))
            
            #print('Y hat ', Yhat_train)
            #print('Y new', Ynew_train)
...
#        torch.save(solver_net.state_dict(), f)
    ...
    if args['probType'] == 'dc_wss':
        ...
        if args['dc'] == 3:
            ...
            input_data = torch.tensor([[1, 4, 7, 10., 14.5, 2, 2.5, 2., 2., 1.]])
            #input_data = np.array(input_data, dtype=np.float32)
...
def eval_net(data, X, solver_net, args, prefix, stats):
    ...
    if args['probType'] == 'dc_wss':
        ...
        Y = solver_net.process_output_differentiable(X, Y)
        Y = solver_net.process_output_sorted(Y)
...
def total_loss(data, X, Y, args, i):
    ...
    if args['probType'] == 'dc_wss':
        # Penalidade de sobreposição
        start_times = Y[:, :5]
        durations = Y[:, 5:]
        lambda_overlap = 1000.0  # Ajuste conforme necessário
        overlap = data.overlap_penalty(start_times, durations)
        result = obj_cost + args['softWeight'] * (1 - args['softWeightEqFrac']) * ineq_cost + lambda_overlap * overlap
    else:
...
def grad_steps(data, X, Y, args, epoch=None):        
    ...
    if take_grad_steps:
        ...
        print(f'Qty iterations: {count_y_steps}')
        return Y_new
...
class NNSolver(nn.Module):
    ...
    def process_output_differentiable(self, x, out):
        # Aplica sigmoide
        out = torch.sigmoid(out)

        # Separa horários e durações
        start_time = out[:, :5] * 23.9
        duration = out[:, 5:] * (5.0 - 0.001) + 0.001

        return torch.cat([start_time, duration], dim=1)

    def process_output_sorted(self, output):
        # Separa
        start_time = output[:, :5]
        duration = output[:, 5:]

        # Ordena horários
        start_sorted, indices = torch.sort(start_time, dim=1)

        # Reordena as durações para corresponder
        duration_sorted = torch.gather(duration, 1, indices)

        return torch.cat([start_sorted, duration_sorted], dim=1)
