...
def plot_nivel_tanque_new(args,
    ...
    Parameters:
    - args: dict containing 'dc' and other settings
    ...
    fig.suptitle(
        f"DC={args['dc']} – Samples={args['qtySamples']}- Epochs={args['epochs']} - Vf {args['vector_format']}- (Total Cost: €{total_cost:.2f}) — {now} - "
    )
    ...
    if save_plot:
        ...
        filename = f"plot_dc_example_dc{args['dc']}_{args.get('vector_format','')}_{now}.png"
        plt.savefig(
