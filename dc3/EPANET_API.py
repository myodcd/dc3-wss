...
def WSS_Components(d, n_links, n_nodes):
    ...
    CP = {
        "CPs_idx": CPs_idx,
        "CPs_id": [x.decode("utf-8") for x in CPs_id],
        "patterns_idx": patterns_idx,
        "patterns_id": [x.decode("utf-8") for x in patterns_id],
    }
    ...
    d.n_max_inc = int(
        (d.T_s / d.timeInc_s) * 2 * d.n_pumps
    )  # numero maximo de incrementos
    pumps = {
        "pumps_idx": pumps_idx,
        "pumps_id": [x.decode("utf-8") for x in pumps_id],
        "pumps_tar_idx": [],
        "pumps_tar_id": [],
    }
...
def EpanetAPIData(
    d, pumps, tanks, pipes, valves, tariffs
    ...
    while t_next > 0:
        ...
        for i in range(d.n_pumps):
            ...
            pumps["pump" + str(i) + "_sp"][n_inc] = em.ENgetlinkvalue(
                idx, em.EN_SETTING
            )
            ...
            pumps["pump" + str(i) + "_h"][n_inc] = em.ENgetlinkvalue(
                idx, em.EN_HEADLOSS
            )
        ...
        for i in range(d.n_valves):
            ...
            valves["valve" + str(i) + "_s"][n_inc] = em.ENgetlinkvalue(
                idx, em.EN_STATUS
            )
        ...
        for i in range(d.n_tanks):
            ...
            tanks["tank" + str(i) + "_h"][n_inc] = em.ENgetnodevalue(
                idx, em.EN_HEAD
            ) - em.ENgetnodevalue(idx, em.EN_ELEVATION)
        ...
        if (
            t_i == 24 * 60 * 60 and d.flag_t_inicio != 0
        ):  # used for Van Zyl network-->simulation started at 7am)
            t_i = 0
            t_inicio.append(t_i)
            t_i += t_next
        else:
    ...
    for i in range(d.n_pumps):
        pumps["pump" + str(i) + "_s"] = np.delete(
            pumps["pump" + str(i) + "_s"], data_dump_p
        )
        pumps["pump" + str(i) + "_q"] = np.delete(
            pumps["pump" + str(i) + "_q"], data_dump_p
        )
        pumps["pump" + str(i) + "_p"] = np.delete(
            pumps["pump" + str(i) + "_p"], data_dump_p
        )
        pumps["pump" + str(i) + "_sp"] = np.delete(
            pumps["pump" + str(i) + "_sp"], data_dump_p
        )
        pumps["pump" + str(i) + "_h"] = np.delete(
            pumps["pump" + str(i) + "_h"], data_dump_p
        )
        pumps["pump" + str(i) + "_eff"] = np.delete(
            pumps["pump" + str(i) + "_eff"], data_dump_p
        )
        pumps["pump" + str(i) + "_pot"] = np.delete(
            pumps["pump" + str(i) + "_pot"], data_dump_p
        )

    for i in range(d.n_valves):
        valves["valve" + str(i) + "_s"] = np.delete(
            valves["valve" + str(i) + "_s"], data_dump_p
        )
        valves["valve" + str(i) + "_q"] = np.delete(
            valves["valve" + str(i) + "_q"], data_dump_p
        )

    for i in range(d.n_pipes):
        pipes["pipe" + str(i) + "_q"] = np.delete(
            pipes["pipe" + str(i) + "_q"], data_dump_p
        )

    for i in range(d.n_tanks):
        tanks["tank" + str(i) + "_h"] = np.delete(
            tanks["tank" + str(i) + "_h"], data_dump_t
        )
    ...
    for inc in range(0, len(timeInc["StartTime"])):
        ...
        for i in range(d.n_pumps):
            pumps["pump" + str(i) + "_tar"].append(
                em.ENgetpatternvalue(pumps["pumps_tar_idx"][i], period)
            )
...
def EpanetSimulation_MultiplosFicheiros(x, d, sim_step):
    ...
    d, pumps, tanks, pipes, valves, timeInc = EpanetAPIData(
        d, pumps, tanks, pipes, valves, tariffs
    )
...
def EpanetSimulation(x, d, sim_step):  # Simulação hidraulica utilizando o epamodule
    ...
    if sim_step != 0:
        em.ENsettimeparam(
            em.EN_HYDSTEP, sim_step
        )  # definir hidraulic step a sim_step seg por causa das dif.finitas
    d.timeInc_s = em.ENgettimeparam(
        em.EN_HYDSTEP
    )  # duração de cada incremento de simulação
    d.T_s = em.ENgettimeparam(0)  # duração do total time horizon
    d.n_tariffs = (
        em.ENgettimeparam(em.EN_PATTERNSTEP) / 60
    )  # duração das tarifas em minutos
    units_flow = em.ENgetflowunits()
    if (units_flow) == 8:
    ...
    d, pumps, tanks, pipes, valves, CP, tariffs = WSS_Components(
        d, n_links, n_nodes
    )  # Definição das estruturas WSS<

    if (
        d.flag_pat != 0 or d.flag_def_pattern == 1
    ):  # alteração dos patterns por otimização ou criação de novo .inp
        # ler patterns
        patterns = pattern_definition(d, CP)
        # definir patterns
        set_patterns(d, patterns, CP)
...
def speed_definition_v2(x, d, pumps):  # VPS -> speed_ini e speed_end
    ...
    for p in range(len(d.pumps_to_opt)):  # .n_pumps):
        ...
        if d.flag_t_inicio != 0:
            ...
            inicio = [
                x + z
                for x, z in zip(
                    np.multiply(st_aux, (60 * 60)), np.multiply(init_time, (60 * 60))
                )
            ]  # inicio_sec, init_time
            ...
            final_sec = [
                x + y for x, y in zip(inicio_sec, np.multiply(dur_aux, (60 * 60)))
            ]  # inicio_sec(com o init_time e arredondado!), dur_sec
            ...
            st_sec = [
                math.floor((st_aux[i] * 60 * 60) + 0.5) for i in range(len(st_aux))
            ]
            final_sec = [
                math.floor(x + y + 0.5)
                for x, y in zip(np.multiply(dur_aux, (60 * 60)), st_sec)
            ]
    ...
    speed_output = {
        ...
        "endt_sec": dc_end,
        "speed": sp,
        "pumps_idx_st": pumpidx_st,
        "pumps_idx_end": pumpidx_end,
    }
...
def conversor_v1_OLD(
    x, pumps, d, speed
):  # conversor variaveis de decisão --> pump status
            ...
            for p in range(0, d.n_pumps):
                ...
                if len(out) != 0:
                    for k in range(0, len(out)):
                        if (
                            s_epanet[out[k]] == 0 and s_epanet[out[k] + 1] == 1
                        ):  # sobreposição
                            todel.append(out[k])
    ...
    if rule_id < n_controls:
        for id in range(rule_id, n_controls + 1):
            em.ENsetcontrol(
                id, em.EN_TIMER, pumps["pumps_idx"][0], float(0), 0, (24 * 60 * 60)
            )
        ...
        if len(x) > np.sum(d.n_dc) * 2:  # com VSPs
            ...
            for p in range(len(time_aux)):
                for i in range(len(time_aux[p])):
                    if (
                        (i >= 1 and i < len(time_aux[p]) - 1)
                        and (time_aux[p][i] != time_aux[p][i - 1])
                        and (sp_aux[p][i] == 0 and sp_aux[p][i + 1] == 0)
                    ):
            ...
            for p in range(len(d.pumps_to_opt)):
                ...
                for i in range(0, int(len(time_epanet) / 2)):
                    ...
                    # end=(x_p[i]+x_p[i+int(len(x_p)/2)])*3600
                        ...
                        (time_epanet[idx - 1] / 3600) + x_p[i + int(len(x_p) / 2)]
                    ) * 3600
                    if end % 1 >= 0.5:
            ...
            if len(idx_dur0) != 0:
                for todel in range(len(idx_dur0)):
                    ...
                    em.ENsetcontrol(
                        rule_id,
                        em.EN_TIMER,
                        speed["pumps_idx"][idx_dur0[todel]],
                        speed["speed"][idx_dur0[todel]],
                        0,
                        speed["st_sec"][idx_dur0[todel]],
                    )
                    controls["t"].append(em.ENgetcontrol(rule_id)[4])
                    controls["controls"].append(speed["speed"][idx_dur0[todel]])
                    controls["pump_idx"].append(speed["pumps_idx"][idx_dur0[todel]])
...
def tariff_definition(d):
    ...
    if d.flag_naive == 1 and weekday == "Friday":  # é tarifas de sábado
        for i in range(0, len(d.tariff_sat_time)):
            aux = np.multiply(
                np.ones(int(d.tariff_sat_time[i] / (5 / 60))), d.tariff_sat[i]
            )
        ...
        for i in range(0, len(d.tariff_sun_time)):
            aux = np.multiply(
                np.ones(int(d.tariff_sun_time[i] / (5 / 60))), d.tariff_sun[i]
            )
            pat_tar = np.concatenate((pat_tar, aux))
        ...
        for i in range(0, len(d.tariff_week_time)):
            aux = np.multiply(
                np.ones(int(d.tariff_week_time[i] / (5 / 60))), d.tariff_week[i]
            )
            pat_tar = np.concatenate((pat_tar, aux))
...
def RealDataPre_Processing(pat, time, pat_name):
    ...
    time_series = pd.DataFrame(
        {"Time": (time - start_of_day).total_seconds(), "Pattern": pat}
    )
